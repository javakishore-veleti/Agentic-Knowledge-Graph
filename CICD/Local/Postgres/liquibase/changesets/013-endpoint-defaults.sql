...
--changeset akg:013-endpoint-defaults splitStatements:false endDelimiter:\n/
--comment endpoint defaults
--rollback empty
...
-- The local filesystem default is applied when the endpoint is seeded, not by an
-- UPDATE here: there is no row to update on a fresh database.
...
-- Example endpoints are not inserted here; they load through Administration.
